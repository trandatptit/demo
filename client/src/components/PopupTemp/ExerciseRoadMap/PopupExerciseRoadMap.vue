...
const handleAssess2Submit = (formData) => {
  ...
  // Show summary
  const diseaseLabels = completeData.diseases
    .map((value) => {
      const option = assess1Questions.value.disease.options.find(
        (opt) => opt.value === value
      );
  ...
  const levelLabel =
    assess1Questions.value.currentLevel.options.find(
      (opt) => opt.value === completeData.currentLevel
    )?.label || "";
  const conditionLabel =
    assess1Questions.value.trainingCondition.options.find(
      (opt) => opt.value === completeData.trainingCondition
    )?.label || "";
  const timeLabel =
    assess1Questions.value.trainingTime.options.find(
      (opt) => opt.value === completeData.trainingTime
    )?.label || "";

  const exerciseLabels = completeData.exercises
    .map((value) => {
      const option = assess2ExerciseOptions.value.find(
        (opt) => opt.value === value
      );
      return option ? option.label : value;
    })
